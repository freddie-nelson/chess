...
func SetupInitialBoard() *[Size][Size]Spot {
	...
	for rank := 0; rank < Size; rank++ {
		for file := 0; file < Size; file++ {
			board[file][rank] = Spot{nil, false, file, rank}
		}
	}
...
func placeBackRank(board *[Size][Size]Spot, rank int, color int) {
	board[0][rank] = Spot{&Piece{color, Rook}, true, 0, rank}
	board[1][rank] = Spot{&Piece{color, Knight}, true, 1, rank}
	board[2][rank] = Spot{&Piece{color, Bishop}, true, 2, rank}
	board[3][rank] = Spot{&Piece{color, Queen}, true, 3, rank}
	board[4][rank] = Spot{&Piece{color, King}, true, 4, rank}
	board[5][rank] = Spot{&Piece{color, Bishop}, true, 5, rank}
	board[6][rank] = Spot{&Piece{color, Knight}, true, 6, rank}
	board[7][rank] = Spot{&Piece{color, Rook}, true, 7, rank}
}

func placePawnRank(board *[Size][Size]Spot, rank int, color int) {
	for file := 0; file < Size; file++ {
		board[file][rank] = Spot{&Piece{color, Pawn}, true, file, rank}
	}
}
